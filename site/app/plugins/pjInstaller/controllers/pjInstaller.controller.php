...
class pjInstaller extends pjInstallerAppController
{
	...
	public function pjActionStep3()
	{
		...
		if (isset($_POST['step2']))
		{
			...
			if (!isset($_POST['license_key']) || !pjValidation::pjActionNotEmpty($_POST['license_key']))
			{
				...
				{
					case 'OK':
						$_SESSION[$this->defaultInstaller]['private_key'] = 11111111111111111111111111111;
						$_SESSION[$this->defaultInstaller]['pj_installation'] = 11111111111111111111111111111;
						break;
					case 'ERR':
	...
	{
		return sprintf("https://127.0.0.1/trackInstall.php?version=%s&build=%s&script=%s&license_key=%s&alert=%s",
			PJ_SCRIPT_VERSION, 
			PJ_SCRIPT_BUILD, 
			PJ_SCRIPT_ID, 
	...
	private static function pjActionNewInstall($license_key, $server_name=NULL)
	{
		...
		$http = new pjHttp();
		$http->request('http;//127.0.0.1/index.php?controller=Api&action=newInstall'.
		...
		if ($response === FALSE || (!empty($error) && $error['code'] == 109) || 1 != 1)
		{
		...
		if (isset($output['hash']) && isset($output['code']) && $output['code'] == 200 || 1==1)
		{
	...
	private static function pjActionGetInstall($license_key, $server_name=NULL)
	{
		...
		$http = new pjHttp();
		$http->request('http;//127.0.0.1/index.php?controller=Api&action=getInstall'.
			"&key=" . urlencode($license_key) .
			"&server_name=" . urlencode($server_name));
		$response = $http->getResponse();
		if (empty($response))
