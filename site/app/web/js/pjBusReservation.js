...
(function (window, undefined){
	...
	BusReservation.prototype = {
        ...
        ticketSelect: function(obj){
		    let self = this;
            var $this = pjQ.$(obj),
                bus_id = pjQ.$(obj).attr('data-bus'),
                seat_map = pjQ.$(obj).attr('data-set'),
                total_tickets = 0,
                seats = '',
            ...
            if ($this.hasClass('bsReturnTicketSelect')) return false;
		...
		init: function (opts) {
			...
			this.$container.on("click.bs", ".bsSelectorLocale", function (e) {
				...
				return false;
			}).on("click.bs", ".switch-btn", function (e) {
                if (e && e.preventDefault) {
                    e.preventDefault();
                }
                var is_return = pjQ.$(this).hasClass('switch-on') ? 'F' : 'T';
                ...
                pjQ.$(this).toggleClass('switch-on');
				...
				hashBang("#!/Seats");
			}).on("click.bs", "div.buttonCheckout button", function (e) {
                let select = pjQ.$(this).closest('div.row').find('select.bsTicketSelect');
                select.find('option:nth-child(2)').prop('selected', true);
                self.ticketSelect(select);
                pjQ.$('button[id*=bsBtnCheckout_]').click();
				...
				var $form = pjQ.$('#bsSelectSeatsForm_' + self.opts.index);
				pjQ.$('select[class*=bsTicketSelect-').each(function( index ) {
					var tickets = parseInt(pjQ.$( this ).val(), 10);
						...
						submitHandler: function (form) {
							...
							pjQ.$.get([self.opts.folder, "index.php?controller=pjFrontEnd&action=pjActionCheck"].join(""), {
								...
								if(data.code == '200')
								{
									...
									document.location.href = pjQ.$('input[name=base_url]').val() + '#!/Seats';
			...
			pjQ.$.get([this.opts.folder, "index.php?controller=pjFrontPublic&action=pjActionGetPaymentForm"].join(""), qs).done(function (data) {
				...
				switch (obj.payment) {
					case 'paypal':
						...
						break;
					case 'liqpay':
						self.$container.find("form[name='bsLiqPay']").trigger('submit');
						break;
					case 'authorize':
