	...
	$(function () {
			...
			$dialogSelect.dialog({
				...
				open: function (){
					if($('#reload_map').val() == '1' )
					{
						...
						$.post("index.php?controller=pjAdminBusStop&action=pjActionGetSeats", $frm.serialize()).done(function (data) {
			...
			$dialogReturnSelect.dialog({
				...
				open: function (){
					if($('#return_reload_map').val() == '1' )
					{
						...
						$.post("index.php?controller=pjAdminBusStop&action=pjActionGetReturnSeats", $frm.serialize()).done(function (data) {
		...
		if ($("#grid").length > 0 && datagrid) {
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "index.php?controller=pjAdminBusStop&action=pjActionUpdate&id={:id}"},
				          {type: "delete", url: "index.php?controller=pjAdminBusStop&action=pjActionDeleteBooking&id={:id}"}
						  ],
				columns: [
				          {text: myLabel.city, type: "text", sortable: false, width:140},
				          {text: myLabel.name, type: "text", sortable: false, width:140},
				          {text: myLabel.address, type: "text", sortable: false, editable: false, width:130},
                                          ],
				dataUrl: "index.php?controller=pjAdminBusStop&action=pjActionGetBusTops",
				dataType: "json",
				fields: ['city', 'name', 'address'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "index.php?controller=pjAdminBusStop&action=pjActionDeleteBookingBulk", render: true, confirmation: myLabel.delete_confirmation},
					   {text: myLabel.exported, url: "index.php?controller=pjAdminBusStop&action=pjActionExportBooking", ajax: false}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "index.php?controller=pjAdminBusStop&action=pjActionSaveBooking&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
		...
		$(document).on("focusin", ".datepick", function (e) {
				...
				o = {
					...
					onSelect: function (dateText, inst) {
						if($this.attr("id") == 'booking_date'){
							...
							$.post("index.php?controller=pjAdminBusStop&action=pjActionChangeDate", frm.serialize()).done(function (data) {
			...
			$grid.datagrid("load", "index.php?controller=pjAdminBusStop&action=pjActionGetBooking", "created", "DESC", content.page, content.rowCount);
			return false;
			...
			$('.bs-loader').css('display', 'block');
			$.get("index.php?controller=pjAdminBusStop&action=pjActionGetLocations&pickup_id=" + $(this).val()).done(function (data) {
			...
			{
				$('.bs-loader').css('display', 'block');
				$.get("index.php?controller=pjAdminBusStop&action=pjActionGetLocations&return_id=" + $(this).val()).done(function (data) {
			...
			if($('#bus_id'). val() != '')
			{
				...
				$('.bs-loader').css('display', 'block');
				$.post("index.php?controller=pjAdminBusStop&action=pjActionGetTickets", frm.serialize()).done(function (data) {
			...
			if($(this). val() != '')
			{
				...
				$('.bs-loader').css('display', 'block');
				$.post("index.php?controller=pjAdminBusStop&action=pjActionGetReturnTickets", frm.serialize()).done(function (data) {
