...
class pjAppController extends pjController {
    ...
    public function beforeFilter() {
        ...
        if (isset($_GET['controller']) ? $_GET['controller'] : Router::getController() != 'pjInstaller') {
...
//                
            ...
            $busArr = pjBusModel::factory()->join('pjMultiLang', "t2.model='pjRoute' AND t2.foreign_id=t1.route_id AND t2.field='title' AND t2.locale='" . $this->getLocaleId() . "'", 'left outer')
                                           ...
                                           ->where("(t1.id IN(" . join(',', $busIdArr) . "))")
                                           ->index("FORCE KEY (`bus_type_id`)")
                                           ->orderBy("route asc")
                                           ->findAll()
                                           ->getData();
                ...
                foreach ($locations as $key => &$location) {
                    ...
                    if (isset($busStops[$cityId])) {
                        ...
                        $tmpBusStopData = $pjBusStopModel->reset()
                                ...
                                ->join('pjBusStop', "t4.id=t1.bus_stop_id", 'left outer')
                                ->select("t1.bus_stop_id,CONCAT(t2.content,', ',t3.content) as address,CONCAT(t4.name,', ',t4.address) as address_ukr")
