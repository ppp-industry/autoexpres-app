...
class pjAdminBusTypes extends pjAdmin {
    ...
    public function pjActionUpdate() {
        ...
        if ($this->isAdmin() || $this->isEditor()) {
            ...
            $busOptions = $pjBusTypeOptionItemModel->findAll()->getData();
            $pjBusTypeOptionModel = pjBusTypeOptionModel::factory();
            
            $currentOptions = array_column(
                $pjBusTypeOptionModel->where('bus_type_id', isset($_GET['id']) ? $_GET['id'] : $params['id'])->findAll()->getData(),
                'option_id'
            );
            
            $this->set('busOptions', $busOptions);
            ...
            if (isset($params['bus_type_update'])) {
                $pjBusTypeModel = pjBusTypeModel::factory();
                $pjSeatModel = pjSeatModel::factory();

                $arr = $pjBusTypeModel->find($params['id'])->getData();
                        ...
                        if (is_writable('app/web/upload/bus_types')) {
                            ...
                            if ($Image->getErrorCode() !== 200) {
                                ...
                                if ($Image->load($_FILES['seats_map'])) {
                                    ...
                                    if ($resp['status'] === true) {
                                        $hash = md5(uniqid(rand(), true));
                                        $image_path = PJ_UPLOAD_PATH . 'bus_types/' . $params['id'] . '_' . $hash . '.' . $Image->getExtension();
                            ...
                            $pjSeatModel->where('bus_type_id', $params['id'])->eraseAll();
                            if (isset($params['seats_count'])) {
                                unset($params['seats_count']);
                            }
                        } else {
                            pjUtil::redirect($_SERVER['PHP_SELF'] . "?controller=pjAdminBusTypes&action=pjActionUpdate&id=" . $params['id'] . "&err=ABT11");
                        }
                    } else if ($_FILES['seats_map']['error'] != 4) {
...
//                            if ($Image->getErrorCode() !== 200) {
                                ...
                                if ($Image->load($_FILES['photo'])) {              
                                    
                                    $resp = $Image->isConvertPossible();
                                    
                                    if ($resp['status'] === true) {
                                        $hash = md5(uniqid(rand(), true));
                                        $image_path = PJ_UPLOAD_PATH . 'bus_types/photo_' . $params['id'] . '_' . $hash . '.' . $Image->getExtension();
//                                   
                                        $Image->loadImage();
//                                                                                echo __LINE__;exit();
                                        $Image->saveImage($image_path);
...
//                    echo __LINE__;exit();
                    ...
                    foreach($params['options'] as $optionId){
                        ...
                        $pjBusTypeOptionModel->reset()->setAttributes([
                            'option_id' => $optionId,
                            'bus_type_id' => $params['id'],
                        ])->insert();
                    }
                    
                }
                if (isset($params['seats'])) {
                    $seat1_arr = array_values($pjSeatModel->where('bus_type_id', $params['id'])->findAll()->getDataPair('id', 'id'));
                    $seat2_arr = array();
                    $sdata = array();
                    foreach ($params['seats'] as $seat) {
                        list($id, $sdata['width'], $sdata['height'], $sdata['left'], $sdata['top'], $sdata['name']) = explode("|", $seat);
                        $seat2_arr[] = $id;
                        $sdata['bus_type_id'] = $params['id'];
                ...
                if (isset($params['seats_new'])) {
                    $sdata = array();
                    foreach ($params['seats_new'] as $seat) {
                ...
                if (!isset($params['seats']) && !isset($params['seats_new']) && isset($params['seats_count'])) {
                    $cnt_seats = $pjSeatModel->reset()->where('bus_type_id', $params['id'])->findCount()->getData();
                    if ($params['seats_count'] > $cnt_seats) {
                        for ($i = $cnt_seats + 1; $i <= $params['seats_count']; $i++) {
                            $sdata = array();
                            $sdata['bus_type_id'] = $params['id'];
                            $sdata['name'] = $i;
                            $pjSeatModel->reset()->setAttributes($sdata)->insert();
                        }
                    } else if ($params['seats_count'] < $cnt_seats) {
                        $pjSeatModel->where('bus_type_id', $params['id'])->where("(name > " . $params['seats_count'] . " AND name <= $cnt_seats)")->eraseAll();
                    }
                }
                ...
                $pjBusTypeModel->reset()->where('id', $params['id'])->limit(1)->modifyAll(array_merge($params, $data));

                if (isset($params['i18n'])) {
                    pjMultiLangModel::factory()->updateMultiLang($params['i18n'], $params['id'], 'pjBusType', 'data');
                }

                pjUtil::redirect($_SERVER['PHP_SELF'] . "?controller=pjAdminBusTypes&action=pjActionUpdate&id=" . $params['id'] . "&err=ABT01");
            } else {
                $pjMultiLangModel = pjMultiLangModel::factory();

                $arr = pjBusTypeModel::factory()->find(isset($_GET['id']) ? $_GET['id'] : $params['id'])->getData();
