	...
	public function pjActionGetSchedule()
	{
		...
		if ($this->isXHR())
		{
			...
			if (isset($_GET['bus_id']) && !empty($_GET['bus_id']))
			{
				...
				$pjBusModel->where("t1.route_id IN(".$bus_id.")");
			}
				...
				if((int) $bus_id > 0 && $pjBusModel->where('t1.id', $bus_id)->findCount()->getData() > 0)
				{
					if(isset($_GET['date']))
					{
						...
						if(pjUtil::checkDateFormat($date))
						{
							...
							foreach($b_arr as $v)
							{
								$v['seats'] = $pjBookingSeatModel->reset()->join('pjSeat', "t2.id=t1.seat_id", 'left outer')
																->select("t1.seat_id, t2.name")
																->where("t1.booking_id", $v['id'])
																->findAll()->getDataPair("seat_id", 'name');			
								if(isset($bs_arr[$v['seat_id']]))
								{
									$can_added = true;
									$_arr = array();
