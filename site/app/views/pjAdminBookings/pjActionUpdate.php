...
if (isset($tpl['status']))
{
	...
	<div class="ui-tabs ui-widget ui-widget-content ui-corner-all b10">
		<ul class="ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all">
			...
			<li class="ui-state-default ui-corner-top ui-tabs-active ui-state-active"><a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminBookings&amp;action=pjActionUpdate&amp;id=<?php echo $tpl['arr']['id']; ?>"><?php __('lblUpdateBooking'); ?></a></li>
		</ul>
	</div>
	<?php
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminBookings&amp;action=pjActionUpdate" method="post" class="form pj-form" id="frmUpdateBooking">
		...
		<div id="tabs">
				...
				<div id="seatsBox" style="display: <?php echo !empty($tpl['bus_type_arr']['seats_map']) ? 'block' : 'none';?>;">
					<p>
						...
						<span class="inline-block">
							...
							<input type="hidden" id="selected_seats" name="selected_seats" value="<?php echo join("|", $tpl['seat_pair_arr'])?>"<?php echo !empty($tpl['bus_type_arr']['seats_map']) ? (!empty($tpl['seat_pair_arr']) ? ' class="required"' : null): null;?> />
						</span>
					</p>
				</div>
				<div id="selectSeatsBox" style="display: <?php echo empty($tpl['bus_type_arr']['seats_map']) ? 'block' : 'none';?>;">
					<p>
						<label class="title"><?php __('lblSeats'); ?>:</label>
						<span class="inline-block">
							<span class="block b5">
								<select name="assigned_seats[]" id="assigned_seats" class="pj-form-field<?php echo empty($tpl['bus_type_arr']['seats_map']) ? ' required' : null;?>" multiple="multiple" size="5">
									<?php
			...
			<div id="tabs-2">
                ...
                <p>
                    <label class="title"><?php __('lblPassport'); ?></label>
					<span class="inline-block">
						<input type="text" name="passport" id="passport" class="pj-form-field w250<?php echo $tpl['option_arr']['o_bf_include_passport'] == 3 ? ' required' : NULL; ?>" value="<?php echo htmlspecialchars(stripslashes($tpl['arr']['passport'])); ?>" />
					</span>
                </p>
                <p>
                    <label class="title"><?php __('lblProdLocation'); ?></label>
					<span class="inline-block">
						<input type="text" name="prod_location" id="prod_location" class="pj-form-field w250<?php echo $tpl['option_arr']['o_bf_include_prodlocation'] == 3 ? ' required' : NULL; ?>" value="<?php echo htmlspecialchars(stripslashes($tpl['arr']['prod_location'])); ?>" />
					</span>
                </p>
				<p>
